    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for(int i = 0; i < countTestReadersBook; i++)
            {
                ...
                readersBooks.Add(new ReadersBook(DateTime.Parse($"{i+1}.10.2022"), books[i], readers[i]));
            }

            for(int j = countTestReadersBook; j < books.Count; j++)
            {
                readersBooks.Add(new ReadersBook(DateTime.Parse($"{j}.10.2022"), books[j], emptyReader));
            }
